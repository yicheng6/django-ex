...
def fetch(id=''):
	...
	while True:
		if not crawl(key):
			logger.info('stop:%s'%key)
			break
		key = key - 1
	logger.info('finish:%s'%key)
...
def crawl(key=None):
	try:
		...
		if msg == None:
			...
			price = result['price']
			if average == '0.0':
				logger.info('msg:%s average is 0.0'%id)
			elif len(author) == 0:
				logger.info('msg:%s author not exists'%id)
			elif len(tags) == 0:
				logger.info('msg:%s tags not exists'%id)
			elif isbn == None:
				logger.info('msg:%s isbn13 not exists'%id)
			else:
				tagStrs = []
				for tag in tags:
